...
{
    public class ViewModelVisualizarInspecoes : ViewModelBase
    {
        ...
        public ViewModelVisualizarInspecoes(Action<bool, string> RetornoPesquisar)
        {
        ...
        private async Task<List<Inspecao>> GetDados()
        {
            List<Inspecao> lstDados = new List<Inspecao>();
            try
            {
                string url = string.Concat(App.__EnderecoWebApi, "/Inspecao/RetornarInspecoes");
                ...
                using (HttpClient requisicao = new HttpClient())
                {
                    ...
                    if (resposta.StatusCode == System.Net.HttpStatusCode.OK)
                    {
                        ...
                        lstDados = Newtonsoft.Json.JsonConvert.DeserializeObject<List<Inspecao>>(conteudo).ToList();
                    }
                }
            }
            catch (Exception ex)
