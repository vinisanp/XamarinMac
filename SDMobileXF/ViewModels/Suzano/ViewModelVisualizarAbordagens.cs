...
{
    public class ViewModelVisualizarAbordagens : ViewModelBase
    {
        private DateTime _dtInicial = DateTime.Today.AddDays(-10);
        private DateTime _dtFinal = DateTime.Today;
        private List<Abordagem> _lista;
        private string _qtdRegistros;
        

        public ViewModelVisualizarAbordagens(Action<bool, string> RetornoPesquisar)
        {
        ...
        private async Task<List<Abordagem>> GetDados()
        {
            List<Abordagem> lstDados = new List<Abordagem>();
            try
            {
                string url = string.Concat(App.__EnderecoWebApi, "/Abordagem/RetornarAbordagens");
                ...
                using (HttpClient requisicao = new HttpClient())
                {
                    ...
                    if (resposta.StatusCode == System.Net.HttpStatusCode.OK)
                    {
                        ...
                        lstDados = Newtonsoft.Json.JsonConvert.DeserializeObject<List<Abordagem>>(conteudo).ToList();
                    }
                }
            }
            catch (Exception ex)
