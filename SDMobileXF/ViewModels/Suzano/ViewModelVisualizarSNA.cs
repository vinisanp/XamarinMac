...
{
    public class ViewModelVisualizarSNA : ViewModelBase
    {
        ...
        public ViewModelVisualizarSNA(Action<bool, string> RetornoPesquisar)
        {
        ...
        private async Task<List<Sna>> GetDados()
        {
            List<Sna> lstDados = new List<Sna>();
            try
            {
                string url = string.Concat(App.__EnderecoWebApi, "/SegurancaNaArea/RetornarSNAs");
                ...
                using (HttpClient requisicao = new HttpClient())
                {
                    ...
                    if (resposta.StatusCode == System.Net.HttpStatusCode.OK)
                    {
                        ...
                        lstDados = Newtonsoft.Json.JsonConvert.DeserializeObject<List<Sna>>(conteudo).ToList();
                    }
                }
            }
            catch (Exception ex)
